...
fillHelix(const size_t i1min, const size_t i1max, const size_t i2min, const size_t i2max)
{
	...
	// temporary variables
	size_t i1, i2, bpIn, u1, u2, j1, j2, u1p, u2p, k1, k2, u1best, u2best, bestBP;
	E_type curE, bestE;
	...
	for (i2=i2max+1; i2-- > i2min;) {
		...
		for (bpIn=0; bpIn < helixConstraint.getMaxBasePairs()-2+1 && (i1+bpIn+1-offset1)<helix.size1()
					 							 				  && (i2+bpIn+1-offset2)<helix.size2(); bpIn++) {
			...
			if ( energy.areComplementary(j1,j2) ) {
				...
				if ( bpIn==0 ) {
					...
					if (! ( energy.areComplementary(j1-1,j2-1) && E_isNotINF( getHelixE( i1, i2, bpIn+2-1) ) ) ) {
						continue; // not complementary -> skip
					}

					// update mfe for split at k1,k2
					curE = std::min( curE,
									 getHelixE( i1, i2, bpIn+2-1)
									 + energy.getE_interLeft(j1-1,j2-1,j1,j2)
					);
				} // more than two base pairs
			} // (j1, j2) complementary

			// store helix energy
			setHelixE( i1-offset1, i2-offset2, bpIn+2, curE );

			// check if full base pair number reached
			if (bpIn+1==helixConstraint.getMaxBasePairs()-2) {

				// find best combination in helx for i1,i2,bp
				size_t bp;
				bestE = E_INF;
				...
				for ( bp= helixConstraint.getMinBasePairs(); bp < helixConstraint.getMaxBasePairs()+1; bp++) {
					...
					curE = energy.getE(i1-offset1, j1, i2-offset2, j2, getHelixE(i1,i2, bp)) + energy.getE_init();
				...
				if (E_isNotINF( bestE )) {
					...
					bestE = getHelixE( i1-offset1, i2-offset2, bp);
					// count true helix
					helixCountNotInf++;
				}
		...
		, const size_t nbp)
{
	...
	for ( size_t bpIn=1+nbp-2; bpIn-- > 0; ) {
		// base case: only left and right base pair present
		if (bpIn==0) {
