...
#include "IntaRNA/PredictorMfeLimStack2dHeuristicSeed.h"
...
PredictorMfeLimStack2dHeuristicSeed( const InteractionEnergy & energy
 ...
 : PredictorMfeLimStack2dHeuristic(energy,output,predTracker)
	, seedHandler( energy, seedConstraint )
	, maxHelixLength( 10 )
{
	if (maxHelixLength < seedConstraint.getBasePairs())  {
		throw std::runtime_error("PredictorMfeLimStack2dHeuristicSeed() : maximal helix length "
				+toString(maxHelixLength)+" < seedBasePairs "+toString(seedConstraint.getBasePairs()));
	}
}
		...
		, const OutputConstraint & outConstraint )
{
	...
	if (!(r1.isAscending() && r2.isAscending()) )
		throw std::runtime_error("PredictorMfeLimStack2dHeuristicSeed::predict("+toString(r1)+","+toString(r2)+") is not sane");
#endif
	...
	// holds the energy for a helix comprising of 'index' number of stacked
	// base pairs followed by a seed
	std::vector< E_type > seedHelixLeftStackedE(maxHelixLength, E_INF);
	...
	for (i2=hybridE_seed.size2(); i2-- > 0;) {
		...
		///////////////////////////////////////////////////////////////////
		// check for all helices
		///////////////////////////////////////////////////////////////////


		// iterate over all allowed lengths of helices
		E_type leftStackingE = 0.0;
		for (size_t helixLength = 0; helixLength < maxHelixLength
				...
				; helixLength++)
		{
			...
			///////////////////////////////////////////////////////////////////
			// check if seed can be part of this helix
			///////////////////////////////////////////////////////////////////

			// check if seed is possible for this left boundary (after seed)
			if ( helixLength+seedHandler.getConstraint().getBasePairs() <= maxHelixLength
					&& E_isNotINF( seedHandler.getSeedE(i1+helixLength,i2+helixLength) ) )
			{
				// get right extension with seed
				w1 = seedHandler.getSeedLength1(i1+helixLength,i2+helixLength)-1; assert(i1+w1 < hybridE.size1());
				w2 = seedHandler.getSeedLength2(i1+helixLength,i2+helixLength)-1; assert(i2+w2 < hybridE.size2());

				// TODO for all stacking extensions followed by a bulge

				rightExt = &(hybridE(i1+helixLength+w1,i2+helixLength+w2));
				// get energy of seed interaction with best right extension
				curE = leftStackingE + seedHandler.getSeedE(i1+helixLength,i2+helixLength) + rightExt->E;
				...
				seedHelixLeftStackedE[helixLength] = E_INF;
			}


			// iterate over all loop sizes w1 (seq1) and w2 (seq2)
			for (w1=1; w1-1 <= energy.getMaxInternalLoopSize1() && i1+helixLength+w1<hybridE_seed.size1(); w1++) {
			for (w2=1; w2-1 <= energy.getMaxInternalLoopSize2() && i2+helixLength+w2<hybridE_seed.size2(); w2++) {
				...
				rightExt = &(hybridE_seed(i1+helixLength+w1,i2+helixLength+w2));
		...
		} // for all helix lengths

		// update mfe if needed (call superclass update routine)
		PredictorMfeLimStack2dHeuristic::updateOptima( i1,curCell->j1, i2,curCell->j2, curCellEtotal, false );
...
traceBack( Interaction & interaction )
{
	...
	if ( ! interaction.isValid() ) {
		throw std::runtime_error("PredictorMfeLimStack2dHeuristicSeed::traceBack() : given interaction not valid");
	}
	if ( interaction.basePairs.size() != 2 ) {
		throw std::runtime_error("PredictorMfeLimStack2dHeuristicSeed::traceBack() : given interaction does not contain boundaries only");
	}
#endif
...
#if INTARNA_IN_DEBUG_MODE
	if ( (j2-i2) > 1 ) {
		throw std::runtime_error("PredictorMfeLimStack2dHeuristicSeed::traceBack() : trace leaves ji<j2 : "+toString(i2)+"<"+toString(j2));
	}
#endif
