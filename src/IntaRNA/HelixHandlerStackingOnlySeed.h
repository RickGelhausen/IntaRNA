...
#ifndef INTARNA_HELIXHANDLERSTACKINGONLYSEED_H
#define INTARNA_HELIXHANDLERSTACKINGONLYSEED_H
...
class HelixHandlerStackingOnlySeed : public HelixHandler {
public:
...
public:
	 ...
	 * destructor
	 */
	virtual ~HelixHandlerStackingOnlySeed();
...
inline
HelixHandlerStackingOnlySeed::HelixHandlerStackingOnlySeed(
		const InteractionEnergy & energy
		, const HelixConstraint & helixConstraint
		, const SeedConstraint & seedConstraint
)
		...
		, helixE_seed_rec( HelixIndex({{ 0, 0, 0 }}))
		, helix_seed()
		, offset1(0)
		, offset2(0)
{
...
{
	return helix_seed(i1-offset1, i2-offset2).first;
}
...
getHelixBP(const size_t i1, const size_t i2) const
{
	return helix_seed(i1-offset1, i2-offset2).second;
}
...
// return helixE_rec[i1][i2][bpInbetween][u1][u2][bp]
	return helixE_seed_rec( HelixIndex({{
...
#endif //INTARNA_HELIXHANDLERSTACKINGONLYSEED_H
