...
fillHelix(const size_t i1min, const size_t i1max, const size_t i2min, const size_t i2max)
{
	...
	if ( i1min > i1max ) throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: i1min("+toString(i1min)+") > i1max("+toString(i1max)+")");
	if ( i2min > i2max ) throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: i2min("+toString(i2min)+") > i2max("+toString(i2max)+")");
	if ( i1max > energy.size1() ) throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: i1max("+toString(i1max)+") > energy.size1("+toString(energy.size1())+")");
	if ( i2max > energy.size2() ) throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: i2max("+toString(i2max)+") > energy.size2("+toString(energy.size2())+")");
	if ( helixConstraint.getMinBasePairs() > helixConstraint.getMaxBasePairs() )
		throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: bpMin("+toString(helixConstraint.getMinBasePairs()) +") > bpMax("+toString(helixConstraint.getMaxBasePairs())+")");
	if ( helixConstraint.getMaxBasePairs() < seedHandler.getConstraint().getBasePairs() )
		throw std::runtime_error("HelixHandlerStackingOnlySeed::fillHelix: no seed can fit helix. HelixMaxBP ("
	...
	offset1 = i1min;
	offset2 = i2min;
	seedHandler.setOffset1(i1min);
	seedHandler.setOffset2(i2min);
